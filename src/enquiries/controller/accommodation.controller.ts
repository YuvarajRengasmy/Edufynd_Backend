  ...
  export let getSingleLoggedAccommodation = async (req, res) => {
    try {
      ...
      if (!logs || logs.length === 0) {
        response(req, res, activity, 'Level-3', 'Single-Logged Accommodation', false, 404, {},"No logs found.");
      }
  
      response(req, res, activity, 'Level-1', 'Single-Logged Accommodation', true, 200, logs, clientError.success.fetchedSuccessfully);
    } catch (err) {
      response(req, res, activity, 'Level-2', 'Single-Logged Accommodation', false, 500, {}, errorMessage.internalServer, err.message);
    }
  }
