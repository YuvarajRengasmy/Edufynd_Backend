...
export let getAllAccommodation = async (req, res, next) => {
    try {
        const data = await Accommodation.find({ isDeleted: false }).sort({ _id: -1 })
...
export let getFilteredAccommodation = async (req, res, next) => {
    try {
        ...
        const accommodationList = await Accommodation.find(findQuery).sort({ createdOn: -1 }).limit(limit).skip(page)
