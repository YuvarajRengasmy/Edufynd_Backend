...
import { EnquiryStatus} from '../../setting/moduleSetting/model/flightEnquiryStatus.model'
...
export let createFlightTicketEnquiry = async (req, res, next) => {
    ...
    if (errors.isEmpty()) {
        try {
            ...
            if (flightDetails.status && flightDetails.status.length > 0) {
                flightDetails.status = flightDetails.status.map((status, index) => {
                    ...
                    if (statusDetails.position === 1) {
                        ...
                        const previousStatus = flightDetails.status.find(
