...
let getAllAgent = async (req, res, next) => {
    try {
        const data = await agent_model_1.Agent.find({ isDeleted: false }).sort({ agentCode: -1 });
...
let deleteAgent = async (req, res, next) => {
    try {
        ...
        (0, commonResponseHandler_1.response)(req, res, activity, 'Level-2', 'Delete-Agent', true, 200, agent, 'Successfully Remove the Agent');
    }
    catch (err) {
...
let createAgentBySuperAdmin = async (req, res, next) => {
    ...
    if (errors.isEmpty()) {
        try {
            ...
            agentDetails.password = await (0, Encryption_1.encrypt)(password);
            agentDetails.confirmPassword = await (0, Encryption_1.encrypt)(confirmPassword);
            agentDetails.createdOn = new Date();
            ...
            (0, commonResponseHandler_1.response)(req, res, activity, 'Level-3', 'Create-Agent-By-SuperAdmin', false, 500, {}, 'Field validation error', err.message);
        }
    }
    else {
...
let getFilteredStudentByAgent = async (req, res, next) => {
    try {
        ...
        const agentList = await agent_model_1.Agent.find(findQuery).sort({ agentCode: -1 }).limit(limit).skip(page).populate('studentId', { name: 1, email: 1, mobileNumber: 1 });
