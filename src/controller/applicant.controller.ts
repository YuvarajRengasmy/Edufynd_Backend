...
export let getAllApplicant = async (req, res, next) => {
  try {
    const data = await Applicant.find({ isDeleted: false }).sort({
      applicationCode: -1,
    });
    response(
      req,
  ...
  if (errors.isEmpty()) {
    try {
      ...
      response(
        ...
        200,
        insertData,
        clientError.success.application
      );
    } catch (err: any) {
      console.log(err);
      response(
  ...
  if (errors.isEmpty()) {
    try {
      ...
      if (!application) {
        ...
        const updatedApplication = await Applicant.findById(
          applicantDetails._id
        );
        const course = updatedApplication.course;
        const last =
          updatedApplication.status[updatedApplication.status.length - 1];
        ...
        if (docs) {
          ...
          const dynamicFilename = `${course}_${timestamp}`;
                                            ...
                                            <tr>
                                                <td class="body" style="padding: 40px; text-align: left; font-size: 16px; line-height: 1.6;">
                                                  ...
                                                  <img src=${docs} alt="Image" width="500" height="300" />
                                                    ...
                                                    <p>Best regards,<br>Afynd Private Limited,<br>Chennai.</p>
                                                </td>
                                            </tr>
                                            <tr>
        ...
        transporter.sendMail(mailOptions, (error, info) => {
          if (error) {
            ...
            res
              .status(201)
              .json({
                message: "You have received an Application Status Notification",
              });
          }
        });
        ...
        res
          .status(201)
          .json({
            message: "Application status has been updated and emails sent.",
...
export let getFilteredApplication = async (req, res, next) => {
  try {
    ...
    const applicantList = await Applicant.find(findQuery)
