   ...
   response(req, res, activity, 'Level-3', 'Chat', false, 500, {}, errorMessage.internalServer, err.message);
    ...
    export const getUserSentChats = async (req, res, next) => {
      try {
        ...
        const userSentChats = await ChatMessage.find({superAdminId,senderType: 'user',}).populate('superAdminId',{name:1,profileImage:1}).populate('staffId',{doctorName:1,profileImage:1}); 
...
export let getDoctorChats = async (req, res, next) => {
  try{
    ...
    const doctorSentDetails = await ChatMessage.find({staffId,senderType: 'staff',}).populate('superAdminId',{name:1,profileImage:1}).populate('staffId',{doctorName:1,profileImage:1});;
...
export let getAllChats = async (req, res, next) => {
  try{
    const chatDetails = await ChatMessage.find({isDeleted: false}).populate('superAdminId',{name:1,profileImage:1}).populate('staffId',{doctorName:1,profileImage:1});
