import { StudentEnquiry, StudentEnquiryDocument } from '../model/studentEnquiry.mode'
...
export let getAllStudentEnquiry = async (req, res, next) => {
    try {
        const data = await StudentEnquiry.find({ isDeleted: false });
...
export let getFilteredStudentEnquiry = async (req, res, next) => {
    try {
        ...
        const studentList = await StudentEnquiry.find(findQuery).sort({ createdAt: -1 }).limit(limit).skip(page)
