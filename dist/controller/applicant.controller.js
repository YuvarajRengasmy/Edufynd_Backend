...
let getAllApplicant = async (req, res, next) => {
    try {
        const data = await application_model_1.Applicant.find({ isDeleted: false });
...
let getFilteredApplication = async (req, res, next) => {
    try {
        ...
        const applicantList = await application_model_1.Applicant.find(findQuery).sort({ createdAt: -1 }).limit(limit).skip(page);
